...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  $sql = "SELECT recieved_status, upload_status FROM products WHERE product_id='$product_id'";
  $result = mysqli_query($link, $sql);
  if ($result !== false) {
  $row = mysqli_fetch_row($result);
  // echo $row[0];
  if($row[0] == $recieved_status){
    $product_id_err = "Product ID already Recieved";
    echo "Product ID already recieved";
  }
  if($row[1] != 'Uploaded'){
    $product_id_err = "Product ID is not uploaded so it can not be recieved";
    echo "Product ID is not uploaded so it can not be recieved";
  }
  else{
    $recieved_status = trim($_POST["recieved_status"]);
  }
...
// Check input errors before inserting in database
    if(empty($product_id_err) && empty($upload_status_err) && empty($upload_date_err) && empty($uploaded_by_err)){
      ...
      $sql = "UPDATE products SET recieved_status = '$recieved_status', recieved_date = '$recieved_date', recieved_by = '$recieved_by' WHERE product_id ='$product_id'";

if ($link->query($sql) === TRUE) {
  echo "Product recieved successfully";
} else {
...
<section class="text-gray-700 body-font">
  <div class="container px-5 py-24 mx-auto">
    <div class="flex flex-col text-center w-full mb-20">
      <h1 class="sm:text-4xl text-3xl font-medium title-font mb-2 text-gray-900">Recieve Product</h1>
      <p class="lg:w-2/3 mx-auto leading-relaxed text-base">You can recieve the product by entering the product ID of the product</p>
      ...
      <table class="table-auto w-full text-left whitespace-no-wrap">
        <thead>
          ...
          <tr>
            ...
            <th class=" py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-200 text-center">Recieved Status</th>
        ...
        <p class="text-xs text-gray-500 mt-3"><?php echo $product_id_err; ?></p>
              <input style = "width: 100%;" class="text-center bg-white rounded focus:outline-none  focus:border-indigo-500 text-base  py-2 mb-4" name="product_id" placeholder="Click here to enter Product ID" type="text" required>
            </td>
            <td class="border-t-2 border-gray-200  ">
        ...
        <p class="text-xs text-gray-500 mt-3"><?php echo $recieved_status_err; ?></p>
              <input readonly style = "width: 100%;" class=" text-center bg-white rounded focus:outline-none  focus:border-indigo-500 text-base  py-2 mb-4"  name="recieved_status" value="Recieved" type="text">
            </td>    
   ...
   <?php
   require_once "config.php";
   $sql = "SELECT count(*) FROM products where recieved_status = 'Recieved'";
  $result = mysqli_query($link, $sql);
  if ($result !== false) {
  $row = mysqli_fetch_row($result);
  //  $total_product =  $row[0];
  ...
  ?>
   <!-- recieved products start -->
<section class="text-gray-700 body-font">
  <div class="container-fluid px-6 py-24 mx-auto">
    <div class="flex flex-col text-center w-full mb-20">
      <h1 class="sm:text-4xl text-3xl font-medium title-font mb-2 text-gray-900">Recieved Products</h1>
      <p class="lg:w-2/3 mx-auto leading-relaxed text-base">This are the products that are recieved at the destination</p>
    </div>
    <div class=" w-full mx-auto overflow-auto">
    <p style="padding: 10px;background: #5DADE2; color: white;" class= "text-center">Total Number of Products Recieved: <?php echo $row[0]; ?></p>
    <p style="padding: 10px;background: red; color: white;" class= "text-center">Total Number of Products Not Recieved: <?php echo $row1[0]-$row[0]; ?></p>
    <table class="table-auto w-full text-left whitespace-no-wrap">
        <thead>
          <tr>
            ...
            <th class="px-3 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-200">Recieved Date</th>                    
          </tr>
        </thead>
        <tbody> 
        <?php
...
$sql = "SELECT * FROM products where recieved_status = 'Recieved' order by product_id desc"; 
    $result = $link->query($sql);
    if ($result->num_rows > 0) {
    // output data of each row
...
?>
    <!-- <form action="" method="post" data-toggle="validator"  class="lg:w-1/2 md:w-2/3 mx-auto"> -->
          ...
          <tr>
            ...
            <td class="border-t-2 border-gray-200  ">
              <input readonly class="bg-white rounded focus:outline-none focus:border-indigo-500 text-base  py-2 mb-4" value="<?php echo $row["recieved_status"]; ?>" name="fees" type="text">
            </td>                      
        
            <td class="border-t-2 border-gray-200 ">
              <input readonly class="bg-white rounded focus:outline-none focus:border-indigo-500 text-base px-4 py-2 mb-4" value="<?php echo $row["recieved_date"]; ?>" name="upload_date" type="text">
            </td>  
        <!-- </form> -->            
          </tr>
